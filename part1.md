...
内容を簡単に説明すると、システム資源の構成を変えようとする命令やシステム資源の構成に動作が依存している命令（「センシティブな命令」と呼ばれる）がユーザモードで実行されるときには、これがトラップされなければならない、ということを言っています（図1）。

![センシティブな命令](figures/part1_fig1)
...
![VMX non Root Mode](figures/part1_fig2)
...
Modeへ戻ってくることをVMExitと呼びます（図3）。

![VMEntry、VMExit](figures/part1_fig3)
...
以上のように、VMX non Root
Modeでのゲストマシン実行とVMExit要因ごとのエミュレーション処理を繰り返すことにより、仮想化環境が実現されます（図4）。

![VT-x のライフサイクル](figures/part1_fig4)
